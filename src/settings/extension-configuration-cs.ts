...
import { Nxrm3PageParser } from "../utils/PageParsing/Nxrm3"
...
export class ExtensionConfigurationStateContentScript extends ExtensionConfigurationState {
    ...
    protected postNxrmServerRegistrations(): void {
        if (this.extensionConfig.sonatypeNexusRepositoryHosts.length > 0) {
            logger.logMessage('Ensuring Page Parsers are registered for NXRM Hosts...', LogLevel.DEBUG, DefaultPageParserRegistry.getCount())
            this.extensionConfig.sonatypeNexusRepositoryHosts.forEach((nxrmHost) => {
                ...
                logger.logMessage('   Registered NXRM3 Page Parser', LogLevel.DEBUG, nxrmHost, DefaultPageParserRegistry.getCount())
            })
        }
    }
