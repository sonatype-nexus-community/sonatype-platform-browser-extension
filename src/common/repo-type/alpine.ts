...
export class AlpineLinuxOrgRepo extends BaseRepo {
    ...
    readonly pathRegex: RegExp = /^(?<releaseName>[^/]*)\/(?<releaseFeed>[^/]*)\/(?<architecture>[^/]*)\/(?<artifactId>[^/#?]*)(\?(?<query>([^#]*)))?(#(?<fragment>(.*)))?$/

    readonly versionDomPath: string = '#package > tbody > tr:nth-child(2) > td'
