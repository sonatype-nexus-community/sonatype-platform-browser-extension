...
const parsePubDev = (url: string): PackageURL | undefined => {
    ...
    if (repoType) {
        ...
        if (pathResult && pathResult.groups) {
            ...
            const pageVersion = getStringBetween($(repoType.versionDomPath).text(),`${pathResult.groups.artifactId} `, ' ')
...
function getStringBetween(str: string, start: string, end: string): string {
